	...
	<div data-role="page">
		...
		<div data-role="content">
			...
			<?php if(isset($status) && $status == 1){ ?>
				<a class="lead" style="color:#088A08;float:left;width:100%;"><?=lang('tracking.cambiosOK');?></a>
				<a class="lead" style="color:#000000;float:left;width:100%;"><?=lang('tracking.sft.cambiosTitle');?><br>
				<?=lang('tracking.sft.fl1');?><?= $fecha_entrega_cliente; ?> <br>
				<?=lang('tracking.sft.fl2');?><?= $franja; ?></a>
			<?php }else if(isset($status) && $status == 0){ ?>
				<a class="lead" style="color:#9d261d;float:left;width:100%;"><?=lang('tracking.sft.cambiosKO');?></a>
			<?php }else{/*do nothing*/} ?>
			...
			<form id="trk_form" method="POST" action="<?= base_url().$this->lang->lang(); ?>/tracking/checkDataTracking/1" data-ajax="false">
			    ...
			    <label class="lead"  for="mail_or_telf"><?=lang('tracking.phone.input2');?></label>
     			...
     			<input id="sndButton" value="Consultar" data-theme="b" type="button">
     			<br/>
