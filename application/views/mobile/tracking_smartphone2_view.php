...
<body>
				...
				<form method="POST" action="<?= base_url().$this->lang->lang(); ?>/tracking/setFranjaChange/1" data-ajax="false">
					...
					<div class="ui-field-contain">
	    	 		...
	    	 		<div class="ui-field-contain">
					    ...
					    <input type="hidden" id="conf_franja" name="conf_franja" value="<?php echo $data['conf_franja'];?>" />
					</div>
	...
	$( document ).ready(function() {
		...
		<?php }?>
		// Rellenamos las franjas disponibles en funcion de la configuracion de franjas y horario
		calculaFranjas();
		
		// En cada cambio de fecha del calendario, recalculamos franjas
		$('#fecha_entrega').change(function(){
			calculaFranjas();
		});
	});
	function calculaFranjas(){
	...
	function cargarComboFranja_ER(){
		vaciarComboFranja();
		$('#id_franja_entrega').append("<option value=\"4\"><?=lang('tracking.franja4');?></option>");
		$('#id_franja_entrega').append("<option value=\"3\"><?=lang('tracking.franja3');?></option>");
		$('#id_franja_entrega').removeAttr('disabled');
		$('#id_franja_entrega').selectmenu('refresh', true);
	}
